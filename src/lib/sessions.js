...
export async function getCurrentSessionFromDB() {
    try {
...
export async function createSession(sessionData) {
    try {
        ...
        const existingSession = existingSessions.find(s => s.name === sessionData.name && s.year === sessionData.year);
        if (existingSession) {
...
export async function setSessionAsCurrent(sessionId) {
    try {
...
export async function updateCurrentSessionStatus() {
    try {
        const sessions = await getAllSessions();
...
export async function initializeDefaultSessions(year = new Date().getFullYear()) {
    try {
        const existingSessions = await getAllSessions();
